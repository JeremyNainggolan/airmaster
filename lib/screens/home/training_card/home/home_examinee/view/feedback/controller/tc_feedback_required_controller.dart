...
class TC_FeedbackRequired_Controller extends GetxController {
  ...
  Future<void> needFeedback() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        historyTraining.clear();
        historyTraining.addAll(data['data'] ?? []);
        log('History Training: $historyTraining');
      } else {
        log('Failed to fetch need feedback data: ${response.statusCode}');
      }
    } catch (e) {
      log('Error fetching need feedback: $e');
    }
  }
  ...
  Future<String> createCertificate() async {
    try {
      ...
      var subject = certificateDate['detail_subject'];
      var trainingType = certificateDate['detail_trainingType'];

      DateTime dates;
      var dateValue = certificateDate["detail_date"];
